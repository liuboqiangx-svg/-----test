...
```
📝 中文意思：
   "Cannot read property 'xxx' of undefined" 直译是：
   "无法从 undefined 身上读取 xxx 属性"
   大白话：你要找一个不存在的东西的属性，当然是找不到的。
...
```
📝 中文意思：
   "Maximum update depth exceeded" 直译是：
   "最大更新深度被超过了"
   大白话：组件一直在更新自己，更新次数太多，浏览器受不了了。
...
```
📝 中文意思：
   "Rendered fewer hooks than expected" 直译是：
   "实际渲染的 hooks 比预期的少"
   大白话：React 记得上次有 5 个 hooks，这次却只看到 4 个，它找不到对应的 state 了。
...
```
📝 中文意思：
   "SyntaxError: Unexpected token" 直译是：
   "语法错误：出现了意料之外的符号"
   大白话：代码写错字了，JavaScript 看不懂你在写什么。
...
| 原则 | 说明 |
|------|------|
| **先解释中文意思** | 先把错误原文翻译成大白话，让用户明白这句话在说什么 |
| **再说人话** | 避免技术术语，或立即解释 |
| **再比喻** | 用生活化、有画面感的比喻帮助理解 |
| **给方案** | 解释完立即提供修复方案 |
| **举一反三** | 顺便提醒类似问题的预防方法 |
