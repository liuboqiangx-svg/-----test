...
class ImageLogger {
  ...
  logRequestSuccess(
    ...
    const entry: LogEntry = {
      ...
      level: "info",
      provider: this.provider,
      message: "Image generation request succeeded",
      duration_ms,
      status_code,
      metadata,
    };
 ...
 * 创建日志实例
 */
export function createImageLogger(provider: string): ImageLogger {
  return new ImageLogger(provider, process.env.NODE_ENV !== "test");
}
